    ...
    def save_env_file(self) -> bool:
        ...
        try:
            ...
            with os.fdopen(fd, "w") as f:
                ...
                # Air-gap mode
                _is_airgap = self.config.airgap.lower() in ("true", "1", "yes")
                f.write("# Air-gap mode\n")
                f.write(f"AIRGAP={self._quote_env_value(self.config.airgap)}\n")
                if _is_airgap:
                    f.write("DO_NOT_TRACK='true'\n")
                else:
                ...
                if _is_airgap:
                    # Air-gap: force cloud provider keys to empty, keep only Ollama
                    f.write("# AI Provider API Keys and Endpoints (air-gap: cloud providers disabled)\n")
                    f.write("OPENAI_API_KEY=''\n")
                    f.write("ANTHROPIC_API_KEY=''\n")
                    f.write("WATSONX_API_KEY=''\n")
                    f.write("WATSONX_ENDPOINT=''\n")
                    f.write("WATSONX_PROJECT_ID=''\n")
                    if self.config.ollama_endpoint:
                        f.write(f"OLLAMA_ENDPOINT={self._quote_env_value(self.config.ollama_endpoint)}\n")
                    f.write("\n")
                else:
                    provider_vars = []
